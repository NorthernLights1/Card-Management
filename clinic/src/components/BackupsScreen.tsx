...
export function BackupsScreen({ onBack, onRestored }: Props) {
  ...
  const chooseDrives = async () => {
    setError(null);
    try { setDrives(await listRemovableDrives()); }
  ...
  const doExport = async () => {
    ...
    try { await exportBackup(dir); setMessage(`Backup exported to ${dir}`); }
    catch (e) { setError(String(e)); }
  };

  const exportCsv = async () => {
    setError(null); setMessage(null);
    const dest = await save({ defaultPath: "patients.csv", filters: [{ name: "CSV", extensions: ["csv"] }] });
  ...
  return (
    <>
      <div className="toolbar">
        ...
        <h1 className="page" style={{ margin: 0 }}>Backups</h1>
      </div>
          ...
          <div style={{ marginTop: 14 }}>
            {drives.length === 0 ? (
              ...
              <div className="result-row" key={d.drive} style={{ gridTemplateColumns: "1fr auto" }}>
                ...
                <button className="primary" onClick={() => pickDrive(d.drive)}>Use this drive</button>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="form-card" style={{ marginBottom: 16 }}>
        ...
        <button className="danger" onClick={startRestore}>Restore from backup…</button>
      </div>

      <div className="form-card">
        <div className="section-title">Activity log</div>
        <p className="muted">Who did what, and when. Identifiers only — no patient details.</p>
        <button onClick={() => readAuditLog().then(setAuditLog).catch((e) => setError(String(e)))}>
          {auditLog === null ? "View activity log" : "Refresh"}
        </button>
        {auditLog !== null && (
          <pre style={{ marginTop: 12, maxHeight: 320, overflow: "auto", background: "var(--surface-2)", border: "1px solid var(--line)", borderRadius: 8, padding: 12, fontSize: 12, whiteSpace: "pre-wrap" }}>
            {auditLog.trim() === "" ? "No activity recorded yet." : auditLog}
          </pre>
        )}
      </div>

      {restore && <RestoreModal state={restore} setState={setRestore} onDone={onRestored} />}
      {showImport && <ImportScreen onClose={() => setShowImport(false)} />}
    </>
  );
}

type RestoreState = { folder: string; username: string; password: string; count: number | null; busy: boolean };

function RestoreModal({ state, setState, onDone }: { state: RestoreState; setState: (s: RestoreState | null) => void; onDone: () => void }) {
  const [error, setError] = useState<string | null>(null);
  ...
  const preview = async () => {
    ...
    } catch (e) { setError(String(e)); setState({ ...state, busy: false }); }
  };

  const apply = async () => {
    setError(null);
    setState({ ...state, busy: true });
    try { await restoreApply(state.folder, state.username.trim(), state.password); onDone(); }
    catch (e) { setError(String(e)); setState({ ...state, busy: false }); }
  };
        ...
        {state.count === null ? (
          <>
            <div className="field">
              ...
              <input value={state.username} onChange={(e) => setState({ ...state, username: e.target.value })} autoFocus />
            </div>
            <div className="field">
            ...
            <div className="form-actions">
              <button className="ghost" onClick={() => setState(null)} disabled={state.busy}>Cancel</button>
          ...
          <>
            <div className="banner warn">
              ...
              Restoring will replace all current data. This cannot be undone (except from the automatic pre-restore snapshot).
            </div>
            <div className="form-actions">
              <button className="ghost" onClick={() => setState(null)} disabled={state.busy}>Cancel</button>
